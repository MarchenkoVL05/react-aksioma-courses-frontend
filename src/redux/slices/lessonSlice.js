...
export const filderByCategory = createAsyncThunk(
  'lesson/filderByCategory',
  async (id) => {
    try {
      const categoryIdObj = {
        categoryId: id,
    ...
    builder
      ...
      .addCase(removeLesson.fulfilled, (state, action) => {
        ...
        state.lessons = state.lessons.filter(
          (lesson) => lesson._id !== action.payload.removedLesson._id
        );
      ...
      .addCase(removeQuestion.fulfilled, (state, action) => {
        state.questions = state.questions.filter(
          (question) => question._id !== action.payload.removedQuestion._id
        );
      })
      // Создать вопрос
